...
很遗憾的是，虽然ShadowRocket有很多优点，例如订阅 subcription/规则 rule/模块 module分离，可以给用户相对便利的跨代理供应商的调度
但是ShadowRocket有缺失很多其竞争对手的性能，例如
####
	•	根据 process ID 制定策略
	•	全面接管系统 DNS
 	•	自动测试修复 main proxy 设定
 	•	代理服务器主动测试逻辑
  
尽管如此，ShadowRocket仍然是综合来看对普通用户友好的代理软件，我们尝试着在这个有瑕疵的环境里改善DNS带来的困扰
